# pylint: disable=import-error,too-many-instance-attributes,too-many-statements,too-many-branches
"""ReID MultiBackend for StrongSORT tracker"""
...
def check_suffix(file='yolov5s.pt', suffix=('.pt',), msg=''):
    ...
    if file and suffix:
        if isinstance(suffix, str):
            suffix = [suffix]
        files_list = file if isinstance(file, (list, tuple)) else [file]
        for f in files_list:
...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='osnet_x0_25_msmt17.pt', device=torch.device('cpu'), fp16=False):
        ...
        if self.pt:  # PyTorch
            ...
            try:
                ...
                raise ImportError("ONNX Runtime not installed. Install with: pip install onnxruntime") from e
            ...
            try:
                ...
                raise ImportError("TensorRT not installed. See: https://developer.nvidia.com/tensorrt") from e
            check_version(trt.__version__, '7.0.0', hard=True)  # require tensorrt>=7.0.0
            if device.type == 'cpu':
                device = torch.device('cuda:0')
            Binding = namedtuple('Binding', ('name', 'dtype', 'shape', 'data', 'ptr'))  # TensorRT binding structure
            ...
            try:
                from openvino.runtime import Core, Layout, get_batch  # pylint: disable=import-outside-toplevel
            except ImportError as e:
                raise ImportError("OpenVINO not installed. Install with: pip install openvino") from e
            ie = Core()
            ...
            try:  # https://coral.ai/docs/edgetpu/tflite-python/#update-existing-tf-lite-code-for-the-edge-tpu
                from tflite_runtime.interpreter import Interpreter, load_delegate  # pylint: disable=import-outside-toplevel
            except ImportError:
                try:
                    import tensorflow as tf  # pylint: disable=import-outside-toplevel
                    Interpreter, load_delegate = tf.lite.Interpreter, tf.lite.experimental.load_delegate
                except ImportError as e:
                    raise ImportError("TensorFlow Lite not installed. Install with: pip install tensorflow") from e
    ...
    def _preprocess(self, im_batch):
        """Preprocess image batch for ReID inference"""
    ...
    def forward(self, im_batch):
        """Forward pass through ReID model"""
